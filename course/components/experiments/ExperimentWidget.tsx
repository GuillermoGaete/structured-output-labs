...
export function ExperimentWidget({ modes, preset = "person", rows = 6, ui, showTable = true }: { modes: ConstraintMode[]; preset?: ExperimentPresetId; rows?: number; ui: ModuleUi; showTable?: boolean }) {
  const locale = useLocale();
  const { presets } = useBackend();
  useExperimentFixtures();
  const base = baseRequest({ ...DEFAULT_PARAMS, preset }, presets);
