...
def test_http_models_and_model_field(monkeypatch):
    ...
    with TestClient(main.app) as client:
        ...
        ok = client.post("/tokenize", json={"text": "Ada", "model": TOY_MODEL_ID})
        assert ok.status_code == 200 and ok.json()["tokenizer_id"] == TOY_MODEL_ID
        assert client.post("/tokenize", json={"text": "Ada", "model": "nope/unknown"}).status_code == 404
        assert client.post("/compile", json={"schema": PRESETS["person"]["schema"], "model": TOY_MODEL_ID}).status_code == 200
